"""DAG that loads fixtures data from MinIO to DuckDB."""
...
from include.utils import derive_table_name
...
def load_data():
    ...
    def load_fixtures_data(obj):
        ...
        for item in minio_client.list_objects(gv.FIXTURES_BUCKET_NAME, recursive=True):
            minio_client.fget_object(gv.FIXTURES_BUCKET_NAME, item.obj, item.obj)
            
            # derive table name from object name
            table_name = derive_table_name(item.obj)

            # use read_csv_auto to load data to duckdb
            cursor.execute(
                f"""CREATE TABLE IF NOT EXISTS {table_name} AS
                SELECT * FROM read_csv_auto('{item.obj}');"""
            )
            os.remove(item.obj)
